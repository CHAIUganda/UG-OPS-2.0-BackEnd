...
const getAllProcurements = async (req, res) => {
  try {
    ...
    if (!user.roles.procurementAdmin) {
      return res.status(400).json({
        message: `${user.fName} ${user.lName} must be a procurement admin in order to access all procurement requests. `,
      });
    }
    ...
    const recurseProcessLeave = async (controller, arr) => {
      if (controller < arr.length) {
        ...
        const requestor = await User.findOne({ _id: staffId });
        if (!requestor) {
          recurseProcessLeave(controller + 1, arr);
        } else {
          const userProgram = await Program.findOne({
            _id: requestor.programId,
          });
          let procurementProgram;
          let procurementProgramShortForm;

          if (!userProgram) {
            procurementProgram = null;
            procurementProgramShortForm = null;
          } else {
            procurementProgram = userProgram.name;
            procurementProgramShortForm = userProgram.shortForm;
          }
